...
function Code128({ value, displayValue = true }: { value: string; displayValue?: boolean }) {
  const barcodeRef = useRef<SVGSVGElement>(null);
  useLayoutEffect(() => {
    if (barcodeRef.current) JsBarcode(barcodeRef.current, value, { format: "CODE128", width: 2, height: 58, margin: 0, displayValue, fontSize: 14 });
  }, [displayValue, value]);
...
export default function Home() {
  ...
  const [partToDelete, setPartToDelete] = useState<{job:Job;part:JobPart} | null>(null);
  ...
  const [mainTheme, setMainTheme] = useState<"light"|"dark">(() => window.localStorage.getItem("plantflow-main-theme-v1")==="dark"?"dark":"light");
  ...
  useEffect(() => {
    if (!createdJobConfirmation) return;
  ...
  const deleteJobPart = async (job: Job, part: JobPart) => {
    try {
      await cloudDataService.deletePartHistory(part.code);
      const remainingParts = (job.parts || []).filter(item => item.id !== part.id);
      const updatedJob: Job = remainingParts.length
        ? { ...job, parts: remainingParts }
        : { ...job, parts: undefined, currentDepartmentId: part.currentDepartmentId, status: part.status, updatedAt: part.updatedAt };
      const next = {
        ...state,
        jobs: state.jobs.map(item => item.id === job.id ? updatedJob : item),
        scans: state.scans.filter(scan => scan.jobNumber !== part.code),
      };
      persist(next);
      setOlderScans(current => current.filter(scan => scan.jobNumber !== part.code));
      setPartToDelete(null);
      setNotice({
        kind: "success",
        title: `${part.code} deleted`,
        detail: remainingParts.length
          ? `${part.name} and its movement history were removed. The remaining part barcodes were not changed.`
          : `The final part and its movement history were removed. Job ${job.jobNumber} is now a standard unsplit job.`,
      });
    } catch (error) {
      setNotice({ kind: "error", title: "Part could not be deleted", detail: error instanceof Error ? error.message : "The cloud deletion did not complete." });
    }
  };
  ...
  const changeMainTheme = (theme:"light"|"dark") => {
    setMainTheme(theme);
    window.localStorage.setItem("plantflow-main-theme-v1",theme);
  };
  ...
  return <div className={`app-shell app-theme-${mainTheme} ${sidebarCollapsed?"sidebar-collapsed":""}`}>
    ...
    <main>
      ...
      {page === "jobs" && <section className="jobs-workspace" ref={activeJobsRef}>
        {jobGroups.map((group,index)=><section className={`panel job-group ${index===0&&jobControlsOpen?"controls-open":""}`} key={group.key}>
          ...
          <JobTable jobs={group.jobs} deptName={deptName} settings={state.settings} detailed highlightDeadlines={state.settings.deadlineHighlighting} showActions={jobActionsOpen} collapsibleActions onToggleActions={()=>setJobActionsOpen(open=>!open)} onPrint={setPrintJob} onPrintPart={(job,part)=>setPrintPart({job,part})} onDeletePart={hasAdministrationAccess?(job,part)=>setPartToDelete({job,part}):undefined} onOpen={setSelectedJob} onSplit={setSplitJob} departments={departments} statuses={statuses} allowDateEditing={canChangeSchedule} onInlineUpdate={updateJobInline} onInlinePartUpdate={updatePartInline}/>
        </section>)}
      </section>}
    ...
    {selectedJob && <OverlayPortal target={jobsFullscreen?activeJobsRef.current:null}>{hasAdministrationAccess?<div className="job-review-stack"><JobEditor key={selectedJob.id} job={selectedJob} departments={departments} statuses={statuses} settings={state.settings} canChangeSchedule={canChangeSchedule} onClose={()=>setSelectedJob(null)} onSave={saveJobOverride} onPrint={()=>{setSelectedJob(null);setPrintJob(selectedJob)}} /><ActiveJobDeleteAction job={selectedJob} onDelete={deleteJobPermanently}/></div>:<JobEditor key={selectedJob.id} job={selectedJob} departments={departments} statuses={statuses} settings={state.settings} canChangeSchedule={canChangeSchedule} onClose={()=>setSelectedJob(null)} onSave={saveJobOverride} onPrint={()=>{setSelectedJob(null);setPrintJob(selectedJob)}} />}</OverlayPortal>}
    ...
    {partToDelete && <OverlayPortal target={jobsFullscreen?activeJobsRef.current:null}><DeletePartDialog job={partToDelete.job} part={partToDelete.part} onClose={()=>setPartToDelete(null)} onDelete={()=>void deleteJobPart(partToDelete.job,partToDelete.part)}/></OverlayPortal>}
...
function ProductionFloorPortal({state,notice,onDismissNotice,onSignOut,onReview,onPrint,onPrintPart,onSplit,onUpdateJob,onUpdatePart}:{state:typeof seedState;notice:Notice;onDismissNotice:()=>void;onSignOut:()=>void;onReview:(job:Job)=>void;onPrint:(job:Job)=>void;onPrintPart:(job:Job,part:JobPart)=>void;onSplit:(job:Job)=>void;onUpdateJob:(job:Job,field:"location"|"status"|"dueDate"|"priority"|"notes",value:string)=>void;onUpdatePart:(job:Job,part:JobPart,field:"location"|"status",value:string)=>void}) {
...
function DeletePartDialog({job,part,onClose,onDelete}:{job:Job;part:JobPart;onClose:()=>void;onDelete:()=>void}) {
  const finalPart=(job.parts?.length||0)===1;
  return <div className="job-editor-overlay" role="dialog" aria-modal="true" aria-label={`Delete part ${part.code}`}><div className="job-editor delete-part-dialog"><div className="editor-head"><div><p className="eyebrow">DELETE JOB PART</p><h2>Delete {part.code}?</h2><p>{part.name}</p></div><button type="button" aria-label="Close delete part confirmation" onClick={onClose}>×</button></div><div className="delete-part-warning"><b>This permanently removes this part and its movement history.</b><span>{finalPart?`Job ${job.jobNumber} will return to a standard unsplit job at this part’s current location and status.`:"The other parts and their barcode numbers will remain unchanged."}</span></div><div className="editor-actions"><span/><div><button type="button" className="secondary" onClick={onClose}>Cancel</button><button type="button" className="part-delete-confirm" onClick={onDelete}>Delete Part</button></div></div></div></div>;
}

function JobEditor({job,departments,statuses,settings,canChangeSchedule,onClose,onSave,onPrint}:{job:Job;departments:Department[];statuses:StatusDefinition[];settings:AppSettings;canChangeSchedule:boolean;onClose:()=>void;onSave:(original:Job,updated:Job,minutesHere:number)=>void;onPrint:()=>void}) {
  ...
  return <div className="job-editor-overlay" role="dialog" aria-modal="true" aria-label={`Review job ${job.jobNumber}`}><form className="job-editor" onSubmit={event=>{event.preventDefault();onSave(job,draft,minutesHere)}}><div className="editor-head"><div><p className="eyebrow">JOB RECORD & MANUAL OVERRIDE</p><h2>Review Job {job.jobNumber}</h2><p>Changes made here override the current production record.</p></div><button type="button" aria-label="Close job record" onClick={onClose}>×</button></div>{job.parts?.length&&<><div className="split-guidance"><b>This job has {job.parts.length} independently tracked parts.</b><span>Each part’s current production position is shown below. Make location and status changes from the expanded Active Jobs row.</span></div><section className="review-parts" aria-label="Tracked job parts"><div className="review-parts-heading"><h3>Tracked parts</h3><span>{job.parts.length} total</span></div><div className="review-parts-list">{job.parts.map(part=><article className="review-part-card" key={part.id}><div className="review-part-identity"><b>{part.code}</b><strong>{part.name}</strong><small>{part.description||job.description}{part.quantity?` · Qty ${part.quantity}`:""}</small></div><dl><div><dt>Department</dt><dd><span className="department-pill">{departments.find(item=>item.id===part.currentDepartmentId)?.name||"Not started"}</span></dd></div><div><dt>Status</dt><dd><span className={`status-pill ${statusTone[part.status]||"slate"}`}>{part.status}</span></dd></div><div><dt>Time here</dt><dd>{formatTrackedTime(part.updatedAt,settings,job.overtime)}</dd></div></dl></article>)}</div></section></>}<div className="editor-grid"><label><span>Job number</span><input required value={draft.jobNumber} onChange={e=>change("jobNumber",e.target.value.toUpperCase())}/></label><label><span>Customer</span><input required value={draft.customer} onChange={e=>change("customer",e.target.value)}/></label><label className="wide"><span>Description</span><textarea required rows={2} value={draft.description} onChange={e=>change("description",e.target.value)}/></label>{!job.parts?.length&&<><label><span>Location</span><select value={draft.currentDepartmentId} onChange={e=>change("currentDepartmentId",e.target.value)}><option value="">Not started</option>{departments.filter(item=>item.enabled).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>Status</span><select value={draft.status} onChange={e=>change("status",e.target.value)}>{statuses.filter(item=>item.enabled||item.name===draft.status).sort((a,b)=>a.order-b.order).map(item=><option key={item.id}>{item.name}</option>)}</select></label></>}<label><span>Due date</span>{canChangeSchedule?<CalendarDatePicker value={draft.dueDate} onChange={value=>change("dueDate",value)}/>:<div className="schedule-readonly">{formatDate(draft.dueDate)}<small>Standard access cannot change dates.</small></div>}</label><label><span>Priority</span><select value={draft.priority} onChange={e=>change("priority",e.target.value)}><option>Standard</option><option>Rush</option><option>Critical</option></select></label><label><span>Time tracking</span><select disabled={!canChangeSchedule} value={draft.overtime?"overtime":"standard"} onChange={e=>setDraft(current=>({...current,overtime:e.target.value==="overtime"}))}><option value="standard">Standard business schedule</option><option value="overtime">Overtime — count continuously</option></select><small>{canChangeSchedule?"Overtime includes evenings, nights, and weekends.":"Standard access cannot change time tracking."}</small></label><label><span>Time in current location</span><div className="time-input"><input type="number" min="0" disabled={Boolean(job.parts?.length)||!canChangeSchedule} value={minutesHere} onChange={e=>setMinutesHere(Number(e.target.value))}/><b>minutes</b></div><small>{!canChangeSchedule?"Standard access cannot change the Time Here clock.":job.parts?.length?"Each part maintains its own Time Here clock.":"Adjusting this changes the “Time Here” clock."}</small></label><label className="wide"><span>Production notes</span><textarea rows={3} value={draft.notes} onChange={e=>change("notes",e.target.value)}/></label></div><div className="editor-summary"><span><b>Created</b>{new Date(job.createdAt).toLocaleString()}</span><span><b>Last movement</b>{new Date(job.updatedAt).toLocaleString()}</span><span><b>Route steps</b>{job.route.length}</span></div><div className="editor-actions">{!job.parts?.length?<button type="button" className="secondary" onClick={onPrint}>▥ Reprint Barcode</button>:<span/>}<div><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary">Save Manual Changes</button></div></div></form></div>
}

function StatusPrintSheet({statuses,onClose,onPrint}:{statuses:StatusDefinition[];onClose:()=>void;onPrint:()=>void}) { return <div className="status-sheet-overlay" role="dialog" aria-modal="true"><div className="status-sheet-modal"><div className="reprint-head"><div><p className="eyebrow">LAMINATED STATION COMMANDS</p><h2>Status barcode sheet</h2></div><button onClick={onClose}>×</button></div><div className="status-print-sheet"><img src={worthHigginsLogo} alt="Worth Higgins & Associates"/><h1>PRODUCTION STATUS COMMANDS</h1><p>Scan a status first, then scan one job within 15 seconds.</p><div className="status-label-grid">{statuses.filter(item=>item.enabled).sort((a,b)=>a.order-b.order).map(status=><div className="status-label" key={status.id} style={{borderTopColor:status.color}}><strong>{status.name}</strong><Code128 value={`STATUS:${status.code}`} displayValue={false}/><small>STATUS:{status.code}</small></div>)}</div></div><div className="reprint-actions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" onClick={onPrint}>Print Status Barcodes</button></div></div></div> }
