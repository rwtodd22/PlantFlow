...
export function ReadOnlyPortal({state}:{state:typeof seedState}) {
  ...
  return <div className="viewer-portal">
    ...
    <main className="viewer-main">
      ...
      <div className="viewer-result-note"><b>{visibleJobs.length}</b> matching {visibleJobs.length===1?"job":"jobs"}<span>Current production view · updated in real time</span></div>
      <section className="viewer-groups">{groups.map(bucket=><article className="panel viewer-group" key={bucket.key}><div className="viewer-group-head"><div><h2>{bucket.label}</h2><p>{bucket.jobs.length} {bucket.jobs.length===1?"job":"jobs"}</p></div></div><div className="viewer-table-wrap"><table className="viewer-table"><thead><tr><th className="viewer-star-column"><span className="sr-only">Favorite</span></th><th>Job</th><th>Customer / Description</th><th>Department</th><th>Status</th><th>Priority</th><th>Due</th><th>Time here</th></tr></thead><tbody>{bucket.jobs.map(job=><Fragment key={job.id}><tr className={deadlineTone(job.dueDate,state.settings.deadlineHighlighting)}><td className="viewer-star-column"><button type="button" className={`viewer-star ${starredJobs.includes(job.id)?"selected":""}`} aria-label={starredJobs.includes(job.id)?`Remove job ${job.jobNumber} from starred jobs`:`Star job ${job.jobNumber}`} title={starredJobs.includes(job.id)?"Remove star":"Star this job"} onClick={()=>toggleStar(job.id)}>★</button></td><td><strong>{job.jobNumber}</strong>{job.parts?.length&&<small>{job.parts.length} tracked parts</small>}</td><td><b>{job.customer}</b><small>{job.description}</small></td><td><span className="department-pill">{parentLocation(job,departmentName)}</span></td><td><span className={`status-pill ${statusTone[parentStatus(job)]||"slate"}`}>{parentStatus(job)}</span></td><td><span className={`priority-text ${job.priority.toLowerCase()}`}>{job.priority}</span></td><td>{formatDate(job.dueDate)}</td><td>{timeAgo(parentUpdatedAt(job))}</td></tr>{job.parts?.map(part=><tr className="viewer-part-row" key={part.id}><td className="viewer-star-column"/><td><strong>{part.code}</strong></td><td><b>{part.name}</b><small>{part.description||job.description}{part.quantity?` · Qty ${part.quantity}`:""}</small></td><td><span className="department-pill">{departmentName(part.currentDepartmentId)}</span></td><td><span className={`status-pill ${statusTone[part.status]||"slate"}`}>{part.status}</span></td><td><span className="priority-text">Part</span></td><td>{formatDate(job.dueDate)}</td><td>{timeAgo(part.updatedAt)}</td></tr>)}</Fragment>)}</tbody></table>{!bucket.jobs.length&&<p className="viewer-empty">{scope==="starred"?"No starred jobs yet. Select the star beside a job to add it here.":"No jobs match the current view."}</p>}</div></article>)}</section>
