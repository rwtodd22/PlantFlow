...
4. Export a conventional `public/favicon.ico` too. Safari and some embedded browsers request this file directly even when other favicon links are present.
5. Update the version after `?v=` in `index.html` so browsers immediately request the new icons.
6. Optionally replace `public/favicon.svg` with the same SVG as a fallback for browsers that request the conventional root path.
